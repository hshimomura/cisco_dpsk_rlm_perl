...
Cisco の Catalyst 9800 向け Easy PSK の Feature History では、EasyPSK は Cisco IOS XE Bengaluru 17.5.1 で Early Field Trial 機能として導入されたと記載されています。[^3]
...
### EasyPSK が向いていると考えられるユースケース

EasyPSK は、小規模マンション、学生寮、社宅のような居住系ネットワークでは比較的筋がよい場合があります。例えば次のような条件です。

- ローミングのために SSID は共通にしたい
- 部屋ごと、居住者ごとに Layer 2 は分離したい
- 端末 MAC アドレスを事前登録する運用は現実的でない
- 企業オフィスほど一斉接続や高密度ローミングが起きにくい

このような用途では、部屋ごとに PSK identity を持たせ、部屋ごとの VLAN へ対応付ける設計は運用上成立しやすいです。

ただし、次の問題は残ります。

- WPA2 限定であること
- 部屋単位の PSK を同室者で共有すること
- PSK が漏えいした場合は、その部屋単位で再発行が必要なこと
- 利用者単位の追跡性が弱いこと
- 部屋数や PSK 数が増えるほど RADIUS 側の照合コストが増えること
...
- Cisco の資料では、Catalyst 9800 の EasyPSK は Cisco IOS XE Bengaluru `17.5.1` で初出とされています
- 17.5.x の release notes は 2021年3月31日初版公開のため、Catalyst 9800 での EasyPSK 対応は少なくともその時期までさかのぼれます[^3][^4]
...
- `authorize {}`
  - `rewrite_called_station_id`
  - iPSK/MAB と EasyPSK を併用する場合は `foreach &request:Cisco-AVPair` で `method=mab` と `cisco-bssid=` を見分け、MAB 側と EasyPSK 側に振り分ける
- `authenticate {}`
...
併用時の重要な補足:
iPSK/MAB と Cisco EasyPSK を同じ FreeRADIUS で併用する場合、単純な
`"%{request:Cisco-AVPair[*]}" =~ /cisco-anonce=/`
だけでは十分に堅牢とは言えません。
Catalyst 9800 の EasyPSK request には binary の `cisco-8021x-data` が含まれるため、`Cisco-AVPair[*]` 全体に regex をかけると NUL byte を含む展開で評価に失敗することがあります。

そのため、併用時は `Cisco-AVPair` を 1 件ずつ `foreach` で見て、ASCII で安全に判定できる値だけを拾う構成を推奨します。例えば次のようにします。

- `method=mab` を見つけたら MAB request
- `cisco-bssid=` を見つけたら EasyPSK request
...
- 通常の wireless MAB だけを SQL / PAP または fallback Accept に流す
- EasyPSK request のときだけ `perl_dpsk` / `dpsk` を呼ぶ
- binary を含みうる `Cisco-AVPair[*]` 全体には regex をかけない
...
上の併用構成を採る場合は、ここも `if (&control:Tmp-String-9 == "yes") { ... }` で囲むほうが安全です。
...
上の併用構成を採る場合は、ここも `if (&control:Tmp-String-9 == "yes") { ... }` を使います。
...
[^3]: Cisco Catalyst 9800 Easy PSK の Feature History。Cisco IOS XE Bengaluru 17.5.1 で Early Field Trial 機能として導入。https://www.cisco.com/c/en/us/td/docs/wireless/controller/9800/17-6/config-guide/b_wl_17_6_cg/m_epsk.html
[^4]: Cisco Catalyst 9800 の Cisco IOS XE Bengaluru 17.5.x release notes。初版公開は 2021年3月31日。https://www.cisco.com/c/en/us/td/docs/wireless/controller/9800/17-5/release-notes/rn-17-5-9800.html
